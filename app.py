...
default_path = Path(__file__).parent / "data.csv"
...
if not use_bundled:
	st.error("Bundled data.csv not found. Please add nigel/data.csv and reload the app.")
	st.stop()
try:
	df_raw, used_enc = _read_csv_with_fallback(default_path)
	st.caption(f"Loaded bundled dataset: {default_path} (encoding: {used_enc})")
except Exception as e:
	st.error(f"Failed to read bundled data.csv with fallback encodings: {e}")
	st.stop()
